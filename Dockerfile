...
RUN dpkg --add-architecture armhf && apt-get update && apt-get install -y build-essential git g++-arm-linux-gnueabihf libc6:armhf libstdc++6:armhf flex bison curl # clang lld

RUN apt-get install -y libfreetype-dev pkg-config libx11-dev libfreetype-dev:armhf libx11-dev:armhf
...
RUN dpkg --add-architecture armhf && apt-get update && apt-get install -y build-essential git libc6:armhf libstdc++6:armhf curl libx11-6 libfreetype6 libx11-6:armhf libfreetype6:armhf
